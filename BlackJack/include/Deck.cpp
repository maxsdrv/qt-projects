...
void Deck::Populate() {
	for (int s = static_cast<int>(Card::suit::CLUBS);
		...
		for (int r = static_cast<int>(Card::rank::ACE);
			...
			auto ptr_card = std::make_shared<Card>(static_cast<Card::rank>(r),
...
QString Deck::AdditionalCards(GenericPlayer &aGenericPlayer) {
	std::cout << std::endl;
	...
	return aGenericPlayer.makeLabelString();
}
